 ...
 * <p>
 * Назначает каждой единице измерения вес в зависимости от её
...
public class ConversionNetwork<TWeight extends Number<TWeight>> {
    ...
    /**
     * Строит сеть величин измерения по графу конвертации {@code conversionGraph},
     * начиная с корневого элемента {@code root}.
     *
     ...
     * <p>
     * При запуске {@link Builder#build()}, рекурсивно ходит по графу {@code conversionGraph}
     * и для каждой посещённой вершины запоминает коэффициент её веса относительно корневой вершины.
     */
    ...
    private class Builder {
         ...
         * @throws InfinitelyIncreasingCycleException при наличии в сети циклов с бесконечно увеличивающимся весом.
            ...
            for (ConversionRule<TWeight> rule : conversionGraph.get(measurement)) {
                ...
                if (!weights.containsKey(rule.getSmallPiece())) {
                    ...
                    throw new InfinitelyIncreasingCycleException(
                            "В сети существует бесконечно увеличивающийся цикл с бесконечно увеличивающимся весом"
                    );
                }
            }
        }
