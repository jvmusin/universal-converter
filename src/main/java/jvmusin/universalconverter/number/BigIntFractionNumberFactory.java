...
public class BigIntFractionNumberFactory implements NumberFactory<BigIntFractionNumber> {
    private static final Pattern NUMBER_PATTERN = Pattern.compile("(?<before>\\d*)([.,](?<after>\\d*))?");
     ...
     * <ul>
     * <li>Строка не должна быть равна {@code null};</li>
     * <li>Строка не должна быть пустой или состоять только из разделителя;</li>
     * <li>Разделитель целой и дробной части должен быть точкой или запятой;</li>
     * <li>Разделитель может отсутствовать в записи числа, что означает отсутствие дробной части;</li>
     * <li>Число может иметь максимум один разделитель в своей записи;</li>
     * <li>В строке должна быть хотя бы одна цифра;</li>
     * <li>Число не должно быть отрицательным;</li>
     * <li>Символы, отличные от разделителя, должны быть цифрами.</li>
     * </ul>
     *
    ...
    public BigIntFractionNumber parse(String s) {
        ...
        BigInteger after = afterDot == null || afterDot.isEmpty() ? BigInteger.ZERO : new BigInteger(afterDot);

        return new BigIntFractionNumber(
                before.multiply(shift).add(after),
                shift
        );
    }
}
