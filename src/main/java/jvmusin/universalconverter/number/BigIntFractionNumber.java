 ...
 * <p>
 * В качестве числителя и знаменателя хранит {@link BigInteger}-ы.
 * Предоставляет абсолютную точность за счёт
 * более медленных операций и большим потреблением памяти по сравнению с примитивами.
 */
@Data
public class BigIntFractionNumber implements Number<BigIntFractionNumber> {
    ...
    /**
     * Создаёт {@link BigIntFractionNumber} из числителя {@code numerator} и знаменателя {@code denominator}.
     * <ul>
     * <li>Если знаменатель равен нулю, бросает {@link ArithmeticException};</li>
     * <li>Если числитель равен нулю, то создастся дробь {@code 0/1};</li>
     * <li>Если результирующая дробь отрицательна, то знак минуса в результирующей дроби останется только у числителя;</li>
     * <li>Если числитель и знаменатель не взаимнопросты, то в результрующей дроби они будут сокращены до взаимнопростых.</li>
     * </ul>
     *
